<template>
  <div class="stored">
    <div class="stored-search-box">
      ...
      <div class="stored-search" @click="getDepositFriendData">
    ...
    >
      ...
      >
        ...
        <el-table-column min-width="20%" prop="deposit_time" label="注册时间">
        </el-table-column>
        <el-table-column min-width="30%" prop="invite_me_uid" label="推广用户ID">
        </el-table-column>
        <el-table-column min-width="25%" prop="deposit_amount" label="储值金额">
        </el-table-column>
      </el-table>
      <!-- 如果無數據 -->
...
import { getDepositFriend } from "@/api";
export default {
  name: "stored",
  data() {
    return {
  ...
  mounted() {
    this.getDepositFriendData();
  },
  methods: {
    getDepositFriendData() {
      ...
      getDepositFriend(params)
        .then((res) => {
          if (res.code === 200) {
            let dataList = res.data.data;
            dataList.forEach((item) => {
              item.deposit_time = moment(
                new Date(item.deposit_time * 1000)
              ).format("YYYY-MM-DD");
            });
